...
`NET_PUBLIC_ADR` | auto-detect | Optional. Sets `+net_public_adr` (public address to advertise). Helpful in Docker/NAT so the server appears in the Steam server browser. When `LAN=0`, scripts also set `+sv_hibernate_when_empty 0` to avoid disappearing.
